...
const Section = ({ children }: { children: React.ReactNode }) => (
    <section className="border-t border-gray-200 pt-3">{children}</section>
);
  ...
  return (
    <div id="resume-content" className="bg-white text-gray-800 p-8 font-sans shadow-2xl w-[50rem]">
      ...
      <header className="text-center mb-4">
        ...
        <div className="mt-3 flex justify-center items-center flex-wrap gap-x-5 gap-y-1 text-sm text-gray-600">
        ...
        <div className="mt-2 flex justify-center items-center gap-x-5 text-sm text-gray-600">
      ...
      <section className="mb-4">
        <p className="text-center text-base text-gray-700 leading-relaxed max-w-3xl mx-auto">{aboutData.summary}</p>
      </section>
                  ...
                  <div className="flex justify-between items-baseline">
                      ...
                      {proj.liveUrl !== '#' && 
                        <a href={proj.liveUrl} target="_blank" rel="noopener noreferrer" className="text-sm text-blue-600 hover:underline flex items-center gap-1">
                          Live Demo <LinkIcon className="w-3.5 h-3.5" />
                        </a>
                      }
                  </div>
                  <p className="text-sm text-gray-700 mt-1 leading-relaxed">{proj.description}</p>
                  <div className="flex flex-wrap gap-1.5 mt-2">
                      {proj.tags.map(tag => <span key={tag} className="text-xs bg-gray-100 text-gray-700 px-2.5 py-0.5 rounded-full">{tag}</span>)}
                  </div>
               </div>
            ))}
          </div>
          ...
          <div className="space-y-3">
            {aboutData.education.map((edu, index) => (
              <div key={index} className="pl-6 relative break-inside-avoid">
                ...
                <div className="flex justify-between items-baseline">
                  <p className="text-base font-medium text-blue-600">{edu.institution}</p>
          ...
          <div className="space-y-3">
            {skillData.map((category) => (
               <div key={category.title} className="flex items-start break-inside-avoid">
                  <h3 className="w-1/4 font-semibold text-gray-700 text-sm pr-4 text-right flex items-center justify-end gap-2 pt-1"><category.icon className="w-4 h-4"/>{category.title}</h3>
                  <div className="w-3/4 flex flex-wrap gap-1.5">
                    {category.skills.map((skill) => (
                       <span key={skill} className="text-xs bg-blue-100 text-blue-800 px-2.5 py-0.5 rounded-full">{skill}</span>
                    ))}
                  </div>
               </div>
            ))}
          ...
          <div className="space-y-2">
            {aboutData.certifications.map((cert, index) => (
              <div key={index} className="flex justify-between items-baseline pl-6 break-inside-avoid">
                <div>
                  <a href={cert.url} target="_blank" rel="noopener noreferrer" className="text-base font-semibold text-blue-600 hover:underline">{cert.name}</a>
                  <p className="text-sm text-gray-700">{cert.issuer}</p>
                </div>
                <p className="text-sm text-gray-500 font-mono">{cert.year}</p>
              </div>
            ))}
          </div>
        </Section>
